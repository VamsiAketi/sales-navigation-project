...
describe("cursor local skill sync", () => {
  ...
  it("reports configured Paperclip skills and installs them into the Cursor skills home", async () => {
    ...
    expect(
      (await fs.lstat(path.join(cursorSkillsHomeForAgent(home, "agent-1"), "paperclip"))).isSymbolicLink(),
    ).toBe(true);
  });
  ...
  it("recognizes company-library runtime skills supplied outside the bundled Paperclip directory", async () => {
    ...
    expect(
      (await fs.lstat(path.join(cursorSkillsHomeForAgent(home, "agent-3"), "ascii-heart"))).isSymbolicLink(),
    ).toBe(true);
  });
  ...
  it("keeps required bundled Paperclip skills installed even when the desired set is emptied", async () => {
    ...
    expect(after.entries.find((entry) => entry.key === paperclipKey)?.state).toBe("installed");
    expect(
      (await fs.lstat(path.join(cursorSkillsHomeForAgent(home, "agent-2"), "paperclip"))).isSymbolicLink(),
    ).toBe(true);
  });
});
