...
export const OUTLOOK_OAUTH_POPUP_MESSAGE_TYPE = "paperclip:outlook-oauth";

type OutlookOAuthPopupMessage = {
  type: typeof OUTLOOK_OAUTH_POPUP_MESSAGE_TYPE;
  status: "connected" | "error";
  connectionId?: string;
};

type StartOutlookOAuthPopupInput = {
  companyId: string;
  connectionId: string;
...
const POPUP_NAME = "paperclip-outlook-oauth";
const POPUP_WIDTH = 520;
const POPUP_HEIGHT = 720;
const POPUP_CLOSE_GRACE_MS = 1_000;

function isOutlookOAuthPopupMessage(value: unknown): value is OutlookOAuthPopupMessage {
  if (!value || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
  return (
    record.type === OUTLOOK_OAUTH_POPUP_MESSAGE_TYPE &&
...
export function startOutlookOAuthPopup(input: StartOutlookOAuthPopupInput) {
  void connectorsApi
    .getOutlookOAuthUrl(input.companyId, input.connectionId)
    .then(({ authorizationUrl }) => {
      ...
      const onMessage = (event: MessageEvent) => {
        ...
        if (!isOutlookOAuthPopupMessage(event.data)) return;
        ...
        finish(() => input.onError?.("Outlook connection failed"));
      };
